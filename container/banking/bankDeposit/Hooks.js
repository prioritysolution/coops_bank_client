...
export const useBankDeposit = () => {
  ...
  const { control } = form;
  const { bankAccount, depositDate } = useWatch({ control });

  const handleSubmit = async (values) => {
    postBankDepositApiCall(values);
  };
  ...
  const postBankDepositApiCall = async (item) => {
    ...
    let data = {
      trans_date:
        item.depositDate && item.depositDate.toISOString().slice(0, 10),
  ...
  const getBankAccountApiCall = async (orgId) => {
    ...
    try {
      const res = await getBankAccountAPI(orgId);
  ...
  const getBankBalanceApiCall = async (orgId) => {
    ...
    let data = {
      ...
      date: form.getValues("depositDate").toISOString().slice(0, 10),
      org_id: orgId,
    };
