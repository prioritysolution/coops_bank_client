...
export const useMembershipWithdrawn = () => {
  ...
  const formSchema = yup.object({
    ...
    date: yup
      .date()
      .required("Withdraw date is required")
      .test(
        "is-between",
    ...
    shareBalance: yup.string().required("Rate per share is required"),
    divBalance: yup.string().required("Rate per share is required"),
  ...
  const handleMemberFormSubmit = (values) => {
    ...
    getMembershipWithdrawDataByIdApiCall(data);
  };
  ...
  const getMembershipWithdrawDataByIdApiCall = async (data) => {
    setLoading(true);
    try {
      ...
      if (res.message === "Data Found") {
        ...
        form.setValue("divBalance", res.details[0].Div_Bal);
